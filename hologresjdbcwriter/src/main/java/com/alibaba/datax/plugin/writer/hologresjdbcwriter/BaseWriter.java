    ...
    public static class Task {
        ...
        protected void fillColumn(Put data, TableSchema schema, int index, Column column) throws SQLException {
            ...
            switch (holoColumn.getType()) {
                ...
                default:
                    try {
                        String otherValue = column.asString();
                        if (emptyAsNull && otherValue != null && otherValue.length() == 0) {
                            data.setObject(index, null);
                        } else {
                            data.setObject(index, otherValue);
                        }
                    } catch (Exception e) {
                        throw DataXException
                                .asDataXException(
                                        ...
                                        String.format(
                                                "您的配置文件中的列配置信息有误.尝试了string类型写入.但是失败了. 因为DataX 不支持数据库写入这种字段类型. 字段名:[%s], 字段类型:[%d], 字段Java类型:[%s]. 请修改表中该字段的类型或者不同步该字段.",
                                                holoColumn.getName(),
                                                holoColumn.getType(),
